...
async function search() {
    ...
    try {
        ...
        if (data.data && data.data.products && data.data.products.length > 0) {
            ...
            allProducts = data.data.products.slice();
            ...
            saveLastSearch(query, allProducts, total);
...
function showProducts(products) {
    ...
    const ratings = products.map(p => parseFloat(p.product_star_rating) || 0).filter(r => r > 0);
    const prices = products.map(p => parseFloat((p.product_price || '').replace(/[^0-9.]/g, '')) || 0).filter(pr => pr > 0);
    ...
    products.forEach(p => {
        ...
        const productRating = parseFloat(p.product_star_rating) || 0;
        const productPrice = parseFloat((p.product_price || '').replace(/[^0-9.]/g, '')) || 0;

        const isHighRated = productRating >= avgRating && productRating >= 4.0;
        const isBestPriced = productPrice > 0 && productPrice <= avgPrice;
        const showBadge = isHighRated && isBestPriced;
...
function loadLastSearch() {
    const saved = localStorage.getItem('amazonLastSearch');
    if (saved) {
        lastSearch = JSON.parse(saved);
        if (lastSearch.query) {
            document.getElementById('search').value = lastSearch.query;
        }
        if (lastSearch.results && lastSearch.results.length > 0) {
            allProducts = lastSearch.results;

            // Show filters
            document.getElementById('filters').style.display = 'flex';

            const shuffled = shuffleArray(lastSearch.results.slice());
            const display = shuffled.slice(0, 24);
            showProducts(display);
            const total = lastSearch.totalResults || lastSearch.results.length;
            if (total > 24) {
                document.getElementById('result-count').textContent =
                    `Showing 24 results out of ${total} total`;
            } else {
